...
- App 模型只使用同会话、相邻实际回答间隔在 (0,120] 分钟、完整覆盖且没有相关配置变更的区间。
...
| 主题 | 模型与对比 | 展示规则 |
|---|---|---|
| 手机用量与情绪 | 相邻回答之间 `mood_score ~ intercept + previous_mood + elapsed_minutes + hour_sin + hour_cos + phone_minutes` | h 为回答前主动分钟数的四分位距，且至少 2 分钟 |
...
App 系数是与其他 App 合并参考组的条件关联；不会根据 App 名称推断用户看了什么内容。
...
按固定列顺序，对各列作 RMS 缩放和两次正交化，以相对阈值 `1e-9` 删除冗余控制项，再用 SVD 求解；不逆 `X'X`。目标项冗余、条件数超过 `1e8` 或主模型残余自由度不足则不可估计。返回原始单位的参数和协方差。

不足20个日期用 HC3，明确标注它不能解决日内相关。≥20个日期使用按日聚类 CR1，修正为 `G/(G−1) × (n−1)/(n−p)`；t 自由度为 G−1。HC3 使用残余自由度。杠杆接近1时保留系数而不编造区间。

有≥3个日期时逐日删除，否则有≥3个会话时逐会话删除。h 在重算时固定。每次删除的块、差值、可估计状态和是否同向且非微小均导出。失败重算仍计入分母。删除重算只需系数可识别，不要求额外残余自由度。
...
仅初步升高／降低的 App 进入长期观察的 App 卡片：页面上按同一段累计历史里的总使用时长降序、App ID 升序排列，默认显示前 2 个（没有手机卡片时 3 个），可展开查看全部有明确结果的 App；不用弱结果补位。导出中的 `top_app_finding_ids` 仍按原顺序：先排有删除检查的结果，再按绝对对比差、样本数降序和 App ID 升序，最多3项。不同 App 的 h 必须随结果显示，排序不是因果影响排行榜。

## 数值与工程验证

`AnalysisTest.kt` 用已知结果验证常量总时长、完全共线、等价参数化、HC3、按日聚类 CR1、缺失覆盖、实际回答锚点、夏令时、当天20评分及可复现导出。`MigrationTest.kt` 验证1／2版数据库升级保留事实且不虚构覆盖。正式 JSON 经机器 schema 和跨字段不变量双重校验。
