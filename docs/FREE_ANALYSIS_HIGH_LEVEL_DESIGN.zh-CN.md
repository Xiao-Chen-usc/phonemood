...
状态：已接入 Android 1.4.0。更新日期：2026-09-09。两个模型、分析页及单文件 JSON 导出已实现；正式字段以 [导出2.0](PERIOD_EXPORT_SCHEMA.zh-CN.md) 为准，发布参数以 [统计策略1.0](ANALYSIS_POLICY_V1.zh-CN.md) 为准。下文保留设计推导，候选参数及阶段计划已由这两份实现规格收敛。
...
当前 sourceRevision 每次成功 poll 都变化，不宜直接驱动每次轮询后重算。设计保留快照 sourceRevision 作为来源标识，另由协调层判断触发原因和节流。
...
保留每次实际回答的 `[answer - 30min, answer)` 描述特征，用于 JSON 导出；不再把该窗口的总用时回归作为免费版主要模型。两个模型分别读取：

| 数据集 | 样本单位 | 主要字段 |
|---|---|---|
| PhoneSampleDataset | 相邻两次实际回答的配对 | 前次评分、本次评分、间隔分钟数、回答前 30 分钟主动分钟数、本地小时 |
| AppTransitionDataset | 同会话内相邻实际回答的配对 | 起止 observation_id、起止评分、mood_delta、起止时间、elapsed_ms、区间总主动时间、逐 App 时间、非计入主动时间、覆盖、纳入状态与原因 |

App TransitionBuilder 只能用真实前后回答构造变化，不能从没有回答的检查点虚构基线，也不能用最后 30 分钟使用量解释更长区间的评分变化。
...
建模只使用已记录或可确定派生的数据。没有测得“当天自然心情”、睡眠、压力、线下活动或 App 内内容，不将它们当已知协变量；任何截距都是统计参数，不是无手机情境下的自然情绪。保留两个主要模型，不恢复参考材料的 60 分钟提醒时间窗口、30／40 次评分门槛或日固定效应主模型。

本节原有的每日使用趋势模型和会话内情绪模型因为始终没有出现在分析页而被移除；小节编号保留，以免破坏对 8.4 的引用。
...
情绪模型在至少 3 个实际观测日期时逐日删除；日期不足但至少 3 个会话时逐会话删除；都不足时保留可估计结果并标为缺少跨块验证。

每次删除后重建中心化或控制投影，但保持原比较量 h。无法估计的删除结果计入失败，不能从分母移除而夸大一致性。

```text
R = 与完整样本同方向且超过近零阈值的删除结果数 / 删除块总数
```

候选 R>=0.8 可标为方向较一致的初步倾向；该值是待固定的产品规则，不是 80% 置信度。不满足则使用“倾向暂不一致”的固定模板。近零单位为评分点。对可估计 App 对比计算的 BH FDR 只供内部／导出，不用于阻止早期结果。
...
已确定：手机用量与情绪、App 额外变化两个主要模型；30 分钟近期特征保留导出，App 使用相邻回答区间；各周期独立、最多三个 App、不足不填充、非因果解释、所有文案本地化。以下尚需在实现统计模块前集中固定，不能分散在 UI：
...
先冻结 AnalysisPolicy 与数值方法，固定两个模型的合成参考结果；新增测试：总时长恒定但 App 剩余变化存在仍可估计、目标 App 被控制变量完全解释时不可估计、前后变化与行为同区间、缺少基线不构造 transition。验证结束评分／变化量和 P+G／P+R 参数化得到同一 App 对比，时段敏感性冲突不会触发挑选更强模型，未运行调整时文案不虚称已调整。测试低方差、共线性、不规则配对、日内相关、FDR、少于三个 App、固定排序及七天早期状态。将类型／公式／样本／单位写入统计导出。
