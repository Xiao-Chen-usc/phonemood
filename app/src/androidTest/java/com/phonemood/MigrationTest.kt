...
class MigrationTest {
    @Test fun versionOneFactsSurviveUpgrade() = runBlocking {
        val context = ApplicationProvider.getApplicationContext<Context>()
        val name = "migration-overlay-test.db"
        context.deleteDatabase(name)
        val schema = InstrumentationRegistry.getInstrumentation().context.assets.open("com.phonemood.data.PhoneMoodDatabase/1.json").bufferedReader().use { Json.parseToJsonElement(it.readText()).jsonObject["database"]!!.jsonObject }
        ...
        val upgraded = Room.databaseBuilder(context, PhoneMoodDatabase::class.java, name).addMigrations(MIGRATION_1_2).build()
        try {
