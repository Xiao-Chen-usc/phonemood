...
data class Configuration(val enabled: Boolean = false, val interval: Int = 15, val reset: Int = 5, val excluded: Set<String> = emptySet(), val overlayEnabled: Boolean = true)
class SettingsStore(private val context: Context) {
    ...
    val flow = context.settingsDataStore.data.map { Configuration(it[enabled] ?: false, it[interval] ?: 15, it[reset] ?: 5, it[excluded] ?: emptySet(), it[overlay] ?: true) }
