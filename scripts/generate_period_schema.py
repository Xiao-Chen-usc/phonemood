...
defs['model'] = obj(id=S, outcome=enum('MOOD_SCORE', 'END_MOOD_SCORE'),
...
defs['finding'] = obj(id=S, model_id=S, kind=enum('PHONE_USAGE', 'APP_USAGE'),
    ...
    comparison_unit=const('MINUTES'), contrast_outcome=const('EXTRA_MOOD_CHANGE'))
defs['policy'] = obj(minimum_ratings=const(6), app_minimum_ratings=const(10), max_pair_ms=const(7200000), max_response_latency_ms=const(300000),
    near_zero_mood_points=const(.2), block_consistency_threshold=const(.8),
    app_min_exposed_rows=const(3), app_min_matched_rows=const(6), app_min_comparison_minutes=const(2),
    ...
    rank_tolerance=const(1e-9), contrast_cap_minutes=const(30),
    cluster_min_days=const(20), minimum_blocks_for_deletion_check=const(3), max_condition_number=const(1e8),
    block_selection=const('DAY_IF_AT_LEAST_3_ELSE_SESSION'), daily_drift_sensitivity=const('NOT_IMPLEMENTED_IN_V1'), sampling=S)

schema = obj(record_type=const('phonemood_period_export'), schema_version=const('2.0'), calculation_version=const('2.0'), analysis_version=const('1.1'),
    ...
    measurement=obj(mood_scale=obj(min=const(1), max=const(10), higher_is_better=const(True)), exposure_window_ms=const(1800000),
        ...
        default_settings=obj(mood_interval_minutes=const(15), session_reset_minutes=const(5), excluded_packages=arr(S)),
    ...
    statistical_analysis=obj(policy_version=const('1.1'), models=arr(ref('model')), findings=arr(ref('finding')), top_app_finding_ids=arr(S), policy_snapshot=ref('policy')),
    user_insights=arr(obj(finding_id=S, template_key=S, arguments=obj(difference=nullable(N), comparison_value=nullable(N), comparison_unit=const('MINUTES'), observations=U, app_id=nullable(S)))),
